...
import { REGISTER_URL, LOGIN_URL, TOKEN_URL, LOGOUT_URL, GET_USER } from '../utils/constants';
import { removeAccessToken, removeRefreshToken, setAccessToken, setRefreshToken } from '../utils/auth-tokens';
...
const initialState: AuthState = {
  ...
  export const registerUser = createAsyncThunk<UserResponse, UserData, { rejectValue: ErrorResponse }>(
    'auth/registerUser',
  ...
  export const loginUser = createAsyncThunk<UserResponse, UserData, { rejectValue: ErrorResponse }>(
    'auth/loginUser',
    async (userData: UserData, { rejectWithValue }) => {
      try {
  ...
  export const refreshToken = createAsyncThunk<UserResponse, void, { rejectValue: ErrorResponse }>(
    'auth/refreshToken',
  ...
  export const logoutUser = createAsyncThunk<void, void, { rejectValue: ErrorResponse }>(
    ...
    async (_, { rejectWithValue }) => {
      try {
        ...
        const response = await request<{ success: boolean; message: string }>(LOGOUT_URL, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  export const getUser = createAsyncThunk('auth/getUser', async (_, { rejectWithValue }) => {
    try {
      const response = await authRequest<{ success: boolean; user: { name: string; email: string } }>(GET_USER, {
        method: 'GET',
      });
  ...
  export const updateUser = createAsyncThunk(
    ...
    async ({ name, email, password }: { name: string; email: string; password: string }, { rejectWithValue }) => {
      try {
        const response = await authRequest<{ success: boolean; user: User }>(GET_USER, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        return rejectWithValue('Произошла ошибка при обновлении данных пользователя');
      }
    }
  );
  ...
  const authSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
          ...
          .addCase(refreshToken.fulfilled, (state, action: PayloadAction<UserResponse>) => {
            state.token = action.payload.accessToken;
          })
